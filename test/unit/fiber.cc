...
fiber_test_wait_dead(void)
{
	...
	/*
	 * Here and below we can check for fiber is dead for non joinable
	 * fibers because fiber with standard stack is not freed on death and
	 * we do not start new fibers in between which can reuse dead
	 * fiber memory.
	 */
	fail_unless(fiber_is_dead(fiber1));
	...
	fail_unless(fiber_is_dead(fiber2));

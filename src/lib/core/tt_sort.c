...
struct sort_worker {
	...
	/**
	 * This thread bucket begin index in extra space on bucket sort phase.
	 */
	size_t bucket_begin;
	/**
 ...
 * is `thread_count - 1` omitting most left and most right boundaries. Let's
...
calc_elem_bucket(va_list ap)
{
	...
	struct tt_sort *sort = worker->sort;
 ...
 */
static void
find_splitters(struct tt_sort *sort)
{
	int log2_n = 0;
...
/**
 * Sort all the data without dividing to buckets. Used in case sorting
 * is limited to a single thread.
 */
static int
sort_all(va_list ap)
{
	struct tt_sort *sort = va_arg(ap, typeof(sort));
...
/**
 * See usage description in header.
 *
 * The algorithm idea is to split the data into buckets, each element in
 * bucket `i` is greater than each element in bucket `i - 1`, and then
 * sort the buckets. As a result we will sort original array.
 *
 ...
 * 4. Sort buckets using sequentional qsort_arg and then copy buckets back
 *    to the original array.
 *
 * Steps 2, 3 and 4 are run in parallel on `thread_count` threads. Step 1
 * does not have high computation cost and is run single thread.
 *
 * Additionally we check if data is already sorted before applying main algo.
 *
 * See also:
 * [1] https://en.wikipedia.org/wiki/Samplesort
 * [2] Super Scalar Sample Sort, Peter Sanders and Sebastian Winkel
 *
 * Although this implementation does not use superscalar approach to map
 * elements to buckets as in [2] and use usual binary search.
 */
void
	...
	tt_sort_compare_f cmp, void *cmp_arg, int thread_count)
{
	...
	/*
	 * Upper limit because element to bucket map has unsigned char
	 * storage per element.
	 */
	...
	if (thread_count == 1) {
		...
		struct cord cord;
		if (cord_costart(&cord, "sort.worker.0", sort_all,
				 &sort) != 0) {
			diag_log();
			panic("cord_start failed");
		}
	...
	e = clock_monotonic();
	say_verbose("splitting to buckets, time spent: %.3f sec", e - b);
	...
	e = clock_monotonic();
	say_verbose("sorting buckets, time spent: %.3f sec", e - b);
